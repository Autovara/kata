...
BATCH_SIZES = (3, 4, 6)          # files per deep call, front-loaded on the top tier
DEEP_BATCH_CAP = 15000       # per-file cap for the top (batch-A) files
DEEP_BATCH_BUDGET = 47000    # total char budget for the deep batch-A prompt
TAIL_BUDGET = 50000          # wide coverage pass: enough budget to show EVERY tail file
MAX_EMIT = 16
MIN_DESC = 90
...
def _plan(recs):
    """Combine wide coverage with a second look at the bug-dense top files: call 1 deep-audits
    the top 3, call 2 re-audits the top 3 plus files 4-5 (a second stochastic pass where bugs
    concentrate), call 3 is a wide coverage pass over files 6-13 (with enough budget to show
    every one, so breadth on many-contract projects is not lost)."""
    batches = [recs[:3]]
    if len(recs) > 3:
        batches.append(recs[:5])
    if len(recs) > 5:
        batches.append(recs[5:13])
...
def agent_main(project_dir=None, inference_api=None):
    ...
    try:
        ...
        raw = []
        for i, batch in enumerate(_plan(recs)):
            if deadline - time.monotonic() <= 12:
                break
            if i == 0:
                per_cap, budget = DEEP_BATCH_CAP, DEEP_BATCH_BUDGET
            elif len(batch) >= 7:
                budget = TAIL_BUDGET
                per_cap = max(6_000, min(PER_FILE_CAP, budget // len(batch)))
            else:
                per_cap = max(6_000, min(PER_FILE_CAP, MAX_SOURCE_CHARS // len(batch)))
                budget = MAX_SOURCE_CHARS
            try:
                raw.extend(_audit_batch(inference_api, batch, by_base, deadline, per_cap, budget))
            except _Budget:
                break
