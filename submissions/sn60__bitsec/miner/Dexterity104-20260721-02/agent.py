...
# Fully deterministic decoding: the crown swings on run-to-run variance, so we
# sample the same way every replica and reduce residual variance by unioning
# multiple internal passes rather than raising temperature.
TEMPERATURE = 0.0
TRIAGE_MAX_TOKENS = 7_000
DEEP_MAX_TOKENS = 8_000
FOCUS_MAX_TOKENS = 8_000

# Tightened to the king's envelope (~690/reserve) so replicas finish inside the
# wall clock and stop burning invalid runs on slow TEE generations.
GLOBAL_DEADLINE = 690.0
REQUEST_TIMEOUT = 190.0
CALL_RESERVE = 205.0
# The final recheck pass is gated on a smaller reserve: only fast local work
# (probes, normalize, dedupe) follows it, so it should not be skipped just
# because the two earlier calls ran long.
LAST_CALL_RESERVE = 150.0
POST_RESERVE = 12.0
MIN_CALL_TIME = 30.0
MAX_ATTEMPTS = 2
...
SYSTEM = (
    ...
    "identify all distinct HIGH or CRITICAL vulnerabilities you can localize to an "
    "exact function - not only the single worst one, but do NOT invent issues to pad "
    "the list. Missing a real high or critical is the costly mistake; a speculative "
    "wrong candidate wastes a slot and dilutes the set. In scope: "
...
ENUM_TAIL = (
    "Aim for completeness on REAL bugs: report each distinct high or critical you can "
    "concretely localize to an exact function and justify with a real exploit path - "
    "usually 3 to 8 for a project of this size. Do not stop at the first one or two if "
    "others clearly exist, and emit separate findings when one function has separate "
    "issues; but do NOT pad the list with speculative, low-severity, or duplicate "
    "items - a wrong candidate dilutes the set. For each, state briefly why the "
    "existing modifiers or require-checks do NOT prevent it."
)
...
TRIAGE_INTRO = (
    ...
    "(mark those with a lower confidence). "
...
RECHECK_INTRO = (
    "Independent second audit of the SAME core files below. Re-derive every finding "
    "from scratch - do not assume any prior pass was correct or complete. The goal is "
    "COMPLETENESS on real bugs: surface every distinct high or critical, especially the "
    "ones easy to miss on a first read - cross-function and cross-file interactions, "
    "initialization and upgrade seizure, rounding and share-inflation, stale or "
    "manipulable prices, missing signer/owner/authority checks, and callback/reentrancy "
    "ordering. Hold the same strict bar: a concrete exploit path, exact file and "
...
def _build_body(prompt, max_tokens):
    payload = {
        "model": MODEL,
        "messages": [
...
def _request(inference_api, prompt, deadline, max_tokens):
    ...
    body = _build_body(prompt, max_tokens)
    last = None
    attempt = 0
...
def _build_focused_prompt(batch, by_base, budget, intro=FOCUSED_INTRO):
    parts = [intro]
...
def _audit_focused(inference_api, batch, by_base, deadline, budget, intro=FOCUSED_INTRO):
    prompt = _build_focused_prompt(batch, by_base, budget, intro)
...
def _resolve(file_value, recs_by_rel, by_base, hint_fn=""):
    ...
    if r is not None:
        return r
    # Require a path-segment boundary so "Pool.sol" cannot resolve to
    # "StablePool.sol"; a bare basename still matches via the by_base fallback.
    matches = [
...
def _normalize(raw, recs_by_rel, by_base):
    ...
    raw_fn = re.sub(r"\(.*$", "", raw_fn).strip()  # drop any arg list before matching
    raw_fn = raw_fn.split(".")[-1].split("::")[-1].strip()
    ...
    try:
        ...
        conf = CONF_DEFAULT
    # Gentle precision gate for tiebreak insurance: drop only the findings the
    # model itself flagged as weak. Criticals are never gated; this must stay
    # gentle so it does not cost per-project completeness (criterion #1).
    if severity == "high" and conf < CONF_HIGH_MIN:
        return None
    function = raw_fn
...
def _dedupe(items):
    ...
    for f in sorted(items, key=lambda x: (x["severity"] == "critical", float(x["confidence"]),
        ...
        fkey = f["file"].lower()
        if per_file.get(fkey, 0) >= PER_FILE_EMIT_CAP:
            continue
        # Key on a normalized title, not a coarse type tag: two distinct bugs in
        # one function (e.g. reentrancy + rounding) must both survive so a project
        # with several findings in one file can still be fully solved.
        title_key = re.sub(r"[^a-z0-9]+", " ", f["title"].lower()).strip()[:70]
        key = (fkey, f["function"].lower(), title_key)
        if key in seen:
            continue
        seen.add(key)
        out.append(f)
    ...
    try:
        ...
        if deadline - time.monotonic() >= CALL_RESERVE:
            try:
                ...
                pass
        # Self-consistency completeness pass: re-audit the SAME core files (plus a
        # small extension) with an independent lens and UNION the findings. Two
        # looks at the core raise the odds a single run captures ALL of a project's
        # expected findings, which is what lifts a partial run (0.25/0.5) to a full
        # 1.0 and a project from 1/3 to >=2/3 solved.
        if deadline - time.monotonic() >= LAST_CALL_RESERVE:
            recheck = ordered[:RECHECK_FILES]
            if recheck:
                try:
                    raw.extend(_audit_focused(inference_api, recheck, by_base,
                                              deadline, TAIL_BUDGET, RECHECK_INTRO))
                except Exception:
                    pass
