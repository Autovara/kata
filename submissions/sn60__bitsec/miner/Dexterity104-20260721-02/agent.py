...
# Deterministic decoding: temperature 0 so the same source yields the same audit
# each time; coverage is broadened by unioning multiple passes, not by sampling.
TEMPERATURE = 0.0
TRIAGE_MAX_TOKENS = 7_000
DEEP_MAX_TOKENS = 8_000
FOCUS_MAX_TOKENS = 8_000

# Time budget: keep the whole audit comfortably inside the room wall clock so it
# always returns cleanly, even when the model responds slowly.
...
ENUM_TAIL = (
    "Aim for thoroughness on REAL bugs: report each distinct high or critical you can "
...
RECHECK_INTRO = (
    ...
    "THOROUGHNESS on real bugs: surface every distinct high or critical, especially the "
...
def _normalize(raw, recs_by_rel, by_base):
    ...
    try:
        ...
        conf = CONF_DEFAULT
    # Drop only high-severity findings the model itself marked as low confidence;
    # criticals are always kept. Trims weak noise without losing real bugs.
    ...
    try:
        ...
        if deadline - time.monotonic() >= CALL_RESERVE:
            try:
                ...
                pass
        # Second-opinion pass: re-audit the SAME core files (plus a small
        # extension) with an independent lens and UNION the results. Two careful
        # looks at the core files surface more of a contract's real bugs than one.
