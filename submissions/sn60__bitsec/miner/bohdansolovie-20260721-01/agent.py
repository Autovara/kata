...
"""SN60 challenger tuned for project-pass ranking under TEE budgets.

Promotion now weights projects passed first. This agent maximizes reliable
high/critical hits across languages with: (1) map triage, (2) ultra-deep audit
of the hottest files, (3) wide risk-windowed second pass, plus a small set of
high-precision structural supplements. Temperature 0 and never-raise keep
replicas consistent so 2/3 majorities can form.
"""
...
SYSTEM = (
    "You are a competitive smart-contract auditor (Solidity, Vyper, Rust/Solana, "
    "Move, Cairo). Emit only REAL exploitable HIGH/CRITICAL bugs with concrete "
    "attacker steps and material impact, pinned to exact file+function. Skip gas, "
    "style, missing events, and admin-trust-by-design. Be decisive: if the exploit "
    "is clear, report it. Return strict JSON only."
)
...
def agent_main(project_dir: str | None = None, inference_api: str | None = None) -> dict:
    ...
    try:
        ...
        if calls < MAX_CALLS and left(started, RESERVE):
            raw.extend(audit(
                inference_api, ordered[:3], by_base, started,
                mode="ultra-deep-value-path", per_file=PER_DEEP, budget=DEEP_BUDGET,
                compact=False,
            ))
            calls += 1

        if calls < MAX_CALLS and left(started, RESERVE):
            wide = diversify(ordered, ordered[:3], limit=11)
            raw.extend(audit(
...
def normalize(raw: dict[str, Any], rel_map: dict[str, dict[str, Any]]) -> dict[str, Any] | None:
    ...
    if loc.strip() not in rebuilt:
        rebuilt += loc
    try:
        conf = float(raw.get("confidence") if raw.get("confidence") is not None else 0.86)
