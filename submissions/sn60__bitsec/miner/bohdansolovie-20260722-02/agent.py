"""SN60 recall-first challenger vs Dexterity104-20260721-01 (#169).

#173 lost with 0 true positives — precision filters starved recall. This agent
enumerates HIGH/CRITICAL bugs aggressively across languages, keeps three timed
LLM passes under a 750s envelope, and only lightly filters so real findings
survive. Soft confidence defaults + medium→high upgrade restore detection.
"""
...
def agent_main(project_dir: str | None = None, inference_api: str | None = None) -> dict:
    ...
    try:
        ...
        if end - time.monotonic() >= RESERVE:
            try:
                raw.extend(audit(
                    inference_api, order[:DEEP_N], by_base, end,
        ...
        if end - time.monotonic() >= RESERVE:
            try:
                wide = unique(order[4:4 + WIDE_N] + order[:4])
                raw.extend(audit(
                    inference_api, wide[:WIDE_N], by_base, end,
...
def audit(
    ...
    *,
    ...
    for r in batch:
        body = compact(r["text"], each) if compact else r["text"][:each]
