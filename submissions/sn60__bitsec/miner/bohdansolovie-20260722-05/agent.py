"""SN60 PASS-or-bust challenger vs Dexterity104 (#169).

#177: 0 passes, 1/24 TP, 0 invalid, 7% precision. Oversized small-project
prompts plus reasoning tokens starved the findings JSON. This build keeps
0-invalid timing, drops reasoning_effort so max_tokens go to findings,
audits top files with fuller source (not every file in one megaprompt),
and keeps fuzzy localization + lean probes for the LLM judge.
"""
...
ENUM = (
    "Enumerate every distinct high/critical you can pin to a real function with "
    "evidence from the source (often 4-10 when warranted). Prefer bugs with a clear "
    "attacker path and material fund/privilege impact. One finding per vulnerable "
    "function. Do not invent issues absent from the code. For each, state why existing "
    "modifiers/requires do NOT prevent it."
)
...
def agent_main(project_dir=None, inference_api=None):
    ...
    try:
        ...
        if deadline - time.monotonic() >= SAFE:
            try:
                # Prefer denser source on fewer files over a megaprompt that
                # truncates mid-finding (that killed #177 recall).
                n = min(len(ordered), 8 if small else DEEP_N)
                raw.extend(deep_pass(inference_api, ordered[:n], by_base,
                                        deadline, DEEP_CAP, DEEP_BUDGET))
            except Exception:
                pass
        if deadline - time.monotonic() >= SAFE:
            # Overlap top files again for replica stability; add fresh tail files.
            focus = ordered[3:3 + FOCUS_N] + ordered[:3]
            ...
            if focus_u:
                try:
                    raw.extend(focus_pass(inference_api, focus_u[:FOCUS_N], by_base,
                                          deadline, FOCUS_BUDGET))
                except Exception:
                    pass
