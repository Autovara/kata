"""SN60 Bitsec challenger — size-adaptive multi-language auditor.

Promotion order is decided on project PASS first (replica needs ~100%
detection; project needs 2-of-3). Prior attempts (#175–#177) finished with
0 passes. This agent:

* spends the whole wall clock on denser source for fewer high-yield files
* skips map triage on compact repos so deep audits see full bodies
* recovers exact function/contract names for the LLM judge
* keeps invalids near zero under the Phala 840s budget
"""
