"""SN60 Bitsec challenger — PASS holder with precision gate.

#179 reached 1/7 project PASS and matched king on TP (2), then lost on
precision (18% vs king ~42%). This build keeps the tiny-repo PASS path and
fuzzy localization, but emits fewer, higher-evidence findings so precision
clears the king's reign average.
"""
...
DESC_MIN = 60
MIN_CONF = 0.82
# #179 had 1 invalid at 680s — keep headroom, drop late third call sooner.
WALL = 700.0
HTTP_TO = 185.0
RESERVE = 250.0
POST = 10.0
...
AUDITOR = (
    ...
    "Rust (Solana/Anchor, CosmWasm), Move, and Cairo. Report ONLY vulnerabilities "
    "you can defend as truly exploitable HIGH or CRITICAL with concrete attacker "
    "steps — precision matters as much as recall. Prefer a short list of solid "
    "bugs over a long speculative list. Still do not miss an obvious real one. "
    "In scope: theft/loss of funds, insolvency, unauthorized state change, privilege "
    "escalation, permanent DoS/lockup, mint/supply corruption, oracle manipulation, "
    "reentrancy, signature/replay, missing signer/owner/authority. Out of scope: "
    "gas, style, missing events, pure centralization, 'admin can rug' by design. "
    "Reason privately; output one strict minified JSON object only."
)
...
MAP_HDR = (
    ...
    "things: (1) copy 6-10 highest-yield paths into target_files verbatim; (2) "
    "report ONLY high/critical bugs already clearly justified from the map "
    "(confidence >= 0.85). Skip weak signature-only guesses. "
...
DEEP_HDR = (
    ...
    "exploitable state transition, and material impact. Emit only findings with "
    "confidence >= 0.8 and a concrete attacker path that existing checks do NOT "
    "stop. Typically 3-8 strong findings — quality over quantity. "
    + CLASSES + " " + LOCALIZE + " " + JSON_RULES
    + "\nReturn strict JSON only: " + SCHEMA + "\n"
)

WIDE_HDR = (
    "Second-pass audit focused on high-confidence bugs only. Prefer cross-module "
    "interactions, accounting/rounding theft, stale/manipulable prices, access-"
    "control gaps, reentrancy/callbacks, liquidation math, init/upgrade seizure, "
    "signature replay, and missing signer/authority on Rust/Move/Cairo. Omit "
    "anything speculative. Explain why existing checks do NOT stop each bug. "
    + CLASSES + " " + LOCALIZE + " " + JSON_RULES
    + "\nReturn strict JSON only: " + SCHEMA + "\n"
)
...
def normalize(raw: dict[str, Any], by_rel: dict[str, dict[str, Any]],
    ...
    try:
        ...
        conf = 0.7
    # Do not floor weak model scores — that inflated FPs and killed #179 precision.
    if severity != "critical" and conf < MIN_CONF:
        return None
    if severity == "critical" and conf < 0.7:
        return None
    # Judge matching + precision: prefer localized findings.
    if not function and conf < 0.88:
        return None
    evidence = (mechanism + " " + impact + " " + description).strip()
    if len(evidence) < 50:
        return None
    ...
    return {
        ...
        "confidence": max(conf, 0.88) if severity == "critical" else conf,
    }


def _cand(title: str, rel: str, contract: str, function: str,
          mechanism: str, impact: str, confidence: float = 0.86) -> dict[str, Any]:
    return {
...
def probes(recs: list[dict[str, Any]]) -> list[dict[str, Any]]:
    """High-precision structural probes only — broad probes wrecked #179 precision."""
    ...
    for r in recs:
        ...
        if "function initialize" in low and not any(
            ...
            out.append(_cand(
                ...
                0.9))
        if "tx.origin" in low and any(x in low for x in ("require(", "if (", "assert(")):
            out.append(_cand(
        ...
        for fn in _sol_slices(text):
            ...
            if ("external" in sig or "public" in sig) and "nonreentrant" not in joined:
                call_m = re.search(r"\.call\s*\{|\.call\(", b)
                write_m = re.search(r"\b(balances?|shares?)\s*\[", b)
                if call_m and write_m and call_m.start() < write_m.start():
                    out.append(_cand(
                        f"{contract}.{name} - reentrancy via call-before-write",
                        r["rel"], contract, name,
                        "external call happens before balances/shares update with no "
                        "reentrancy guard",
    ...
    try:
        ...
        if not compact and deadline - time.monotonic() >= RESERVE:
            try:
                ...
                targets, hits = _parse_map(_ask(inference_api, prompt, deadline, MAP_TOK))
                raw.extend(hits)
