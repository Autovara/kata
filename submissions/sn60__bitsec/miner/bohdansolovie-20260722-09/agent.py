"""SN60 Bitsec challenger — recall-widened PASS holder.

#181 got 1/7 PASS, matched projects-passed, then lost tier 3 by 0.1 TP
(2 vs king 2.1) with 1 invalid. Crown math: reaching the king's TP (a tie
at tier 3) with 0 invalids wins outright at tier 4 (0 vs ~2.4, margin >2).

So this build needs exactly +1 reliable TP and 0 invalids:
* TP is best-of-3 replicas, so widen coverage — audit more distinct files
  in both passes so at least one replica catches an extra planted bug.
* Higher emit cap so a real finding is never cut before the judge sees it.
* Conservative timing + adaptive per-call cap so every replica finishes.
"""
...
PER_WIDE = 9_000
RELATED = 2_400
# Wider coverage → more planted bugs seen (TP is best-of-3 replicas).
DEEP_N = 9
WIDE_N = 14
EMIT_MAX = 18
DESC_MIN = 40
# #181 still logged 1 invalid at 710s; tighten wall + adaptive call cap for 0 invalids.
WALL = 700.0
HTTP_TO = 178.0
RESERVE = 250.0
POST = 12.0
...
def agent_main(project_dir: str | None = None, inference_api: str | None = None) -> dict:
    ...
    try:
        ...
        if deadline - time.monotonic() >= RESERVE:
            try:
                deep_n = min(len(ordered), len(ordered) if tiny else (10 if compact else DEEP_N))
        ...
        # Second pass reaches files the deep pass did NOT cover, so a replica can
        # find planted bugs outside the top-N — plus 2 top files for stability.
        # Only with clear slack, so we never overrun and log an invalid.
        if not tiny and deadline - time.monotonic() >= RESERVE + THIRD_SLACK:
            try:
