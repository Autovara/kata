"""SN60 Bitsec challenger — #181 core, denser not wider.

#181: 1/7 PASS, 2 TP, 1 invalid → lost tier 3 by 0.1 TP (2 vs king 2.1).
#182: widened DEEP/WIDE into megaprompts → 0 PASS, 0 TP (JSON starvation).

Crown math unchanged: reach king's TP (tie at tier 3) with 0 invalids →
win at tier 4 (0 vs ~2.3). So restore the #181 PASS path exactly, then:
* denser source on FEWER files (never more files than #181)
* second pass covers fresh files the deep pass skipped, but only 6–8 of them
* stricter third-call slack so every replica finishes (0 invalid)
"""
...
PER_WIDE = 10_000
RELATED = 2_400
# #182 taught: more files → truncated findings JSON → 0 TP. Stay at #181 width.
DEEP_N = 7
WIDE_N = 7
EMIT_MAX = 14
DESC_MIN = 40
# #181 logged 1 invalid at 710s; trim wall + demand more slack before call 2.
WALL = 690.0
HTTP_TO = 175.0
RESERVE = 260.0
POST = 12.0
...
def _fn_from_title(title: str, rec: dict[str, Any]) -> str:
    for blob in (title,):
        m = re.search(r"\b([A-Za-z_][A-Za-z0-9_]*)\s*\(", blob)
        if m:
            hit = _fuzzy_fn(m.group(1), rec)
            if hit:
                return hit
        m = re.search(r"\b[A-Za-z_][A-Za-z0-9_]*\.([A-Za-z_][A-Za-z0-9_]*)\b", blob)
        if m:
            hit = _fuzzy_fn(m.group(1), rec)
            if hit:
                return hit
...
def _fn_from_text(blob: str, rec: dict[str, Any]) -> str:
    """Recover a declared function name mentioned in free text (helps LLM-judge TP)."""
    if not blob:
        return ""
    # Prefer backtick-quoted identifiers that match a real function.
    for m in re.finditer(r"`([A-Za-z_][A-Za-z0-9_]*)`", blob):
        hit = _fuzzy_fn(m.group(1), rec)
        if hit:
            return hit
    for m in re.finditer(r"\bfunction\s+`?([A-Za-z_][A-Za-z0-9_]*)`?", blob, re.I):
        hit = _fuzzy_fn(m.group(1), rec)
        if hit:
            return hit
    # Fall back: any token that exactly matches a known function name.
    low_names = {n.lower(): n for n in rec["fnames"]}
    for tok in re.findall(r"\b([A-Za-z_][A-Za-z0-9_]{2,})\b", blob):
        hit = low_names.get(tok.lower())
        if hit:
            return hit
    return ""
...
def normalize(raw: dict[str, Any], by_rel: dict[str, dict[str, Any]],
    ...
    function = (
        _fuzzy_fn(raw_fn, rec)
        or _fn_from_title(title, rec)
        or _fn_from_text(description, rec)
        or _fn_from_text(mechanism, rec)
    )
    real = rec["contracts"]
    ...
    try:
        ...
        conf = 0.7
    # Soft floor from #181 (recall). Never hard-gate like #180.
    if conf < 0.40:
        return None
    conf = max(conf, 0.55)
...
def _dedupe(items: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    out: list[dict[str, Any]] = []
    # Prefer localized + critical + higher conf — mild precision without hard gates.
    for f in sorted(
            items,
            key=lambda x: (
                bool(x.get("function")),
...
def agent_main(project_dir: str | None = None, inference_api: str | None = None) -> dict:
    ...
    try:
        ...
        if deadline - time.monotonic() >= RESERVE:
            try:
                deep_n = min(len(ordered), len(ordered) if tiny else (8 if compact else DEEP_N))
                per = 18_000 if tiny else PER_DEEP
                budget = 55_000 if tiny else DEEP_BUDGET
                prompt = _batch_prompt(DEEP_HDR, ordered[:deep_n], by_base, per, budget)
                raw.extend(_parse_findings(_ask(inference_api, prompt, deadline, DEEP_TOK)))
            except Exception:
                pass

        # Fresh-file second pass (not a re-hash of the top files). Denser per-file
        # source on FEWER files — #182's 14-file wide pass starved the JSON and
        # wiped every TP. Only fire with clear slack for 0 invalids.
        if deadline - time.monotonic() >= RESERVE + THIRD_SLACK:
            try:
                if tiny:
                    # Tiny PASS path: re-audit with a fresh lens over the same set.
                    focus_u = ordered[: min(len(ordered), 8)]
                else:
                    fresh = ordered[deep_n:deep_n + WIDE_N] + ordered[:2]
                    seen: set[str] = set()
                    focus_u = []
                    for r in fresh:
                        if r["rel"] not in seen:
                            seen.add(r["rel"])
                            focus_u.append(r)
                    focus_u = focus_u[:WIDE_N]
                if focus_u:
                    prompt = _batch_prompt(
                        WIDE_HDR, focus_u, by_base, PER_WIDE, WIDE_BUDGET)
                    raw.extend(_parse_findings(
                        _ask(inference_api, prompt, deadline, WIDE_TOK)))
            except Exception:
                pass
