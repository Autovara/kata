"""SN60 Bitsec challenger — exact #181 PASS core, timing-only fix.

Evidence (stop guessing):
  #179/#181: 1 PASS + 2 TP  ← only runs that worked
  #180/#182/#183: 0 PASS + 0 TP ← every "improvement" destroyed recall

King reign avg now ~1.8 TP / ~2.2 invalids. So #181's 2 TP TIES tier 3;
0 invalids (vs ~2.2) WINS tier 4. Do NOT touch prompts, coverage, filters,
or function recovery — only the clock that caused #181's single invalid.
"""
...
EMIT_MAX = 12
DESC_MIN = 40
# #181 had 1 invalid at WALL=710 / THIRD_SLACK=90 / TRIES=2.
# Leave more headroom under the 840s TEE kill; never retry a hung call;
# demand more slack before starting the second LLM call.
WALL = 660.0
HTTP_TO = 175.0
RESERVE = 255.0
POST = 12.0
