"""SN60 Bitsec challenger — #181 scoring shape, hard 0-invalid.

#184 proved the #181 core still works: 1 PASS + 2 TP (ties tiers 1-3 vs
king ~1.9 TP). It still logged 1 invalid, fell through to precision (18%
vs 45%), and lost. Win condition is now mechanical: same PASS+TP with
0 invalids → take tier 4 (0 vs ~2.1 clears >2).

Changes vs #181/#184 — only what forces every replica to finish:
* never run the map call (max 2 LLM calls, usually 1)
* second LLM call ONLY on tiny repos (the PASS path) with big slack
* TRIES=1, shorter wall, mild emit cap 8 (precision insurance if we somehow
  still carry 1 invalid)
"""
...
EMIT_MAX = 8
DESC_MIN = 40
# #184 still had 1 invalid at 660s. Cut calls, not quality of the deep audit.
WALL = 620.0
HTTP_TO = 170.0
RESERVE = 240.0
POST = 12.0
...
def agent_main(project_dir: str | None = None, inference_api: str | None = None) -> dict:
    ...
    try:
        ...
        # NEVER map — that was a third LLM call and a timeout risk. Risk-rank
        # from local scores is enough to order files for the deep pass.
        # One deep audit is the default. A second call is reserved for tiny
        # repos only (where PASS is won) and only with large slack.
        ...
        if tiny and deadline - time.monotonic() >= RESERVE + THIRD_SLACK:
            try:
                focus = ordered[: min(len(ordered), WIDE_N)]
                prompt = _batch_prompt(
                    WIDE_HDR, focus, by_base, PER_WIDE, WIDE_BUDGET)
