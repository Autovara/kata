"""SN60 Bitsec challenger — #184 PASS core + precision emit-3.

Scoreboard truth (king ~16 challenges deep):
  #184: 1 PASS, 2 TP, 1 invalid, 18% prec → tied tiers 1-4, LOST precision
  #185: 0 PASS, 2 TP, 0 invalid, 25% prec → behind on tier 1 (PASS)
  #186: 0 PASS after changing call path + emit-3 → lost PASS

Win path: keep #184's exact audit path (map-for-large, deep, slack wide).
Only change post-LLM selection: require localized function, skip probe
noise when the model already returned findings, EMIT_MAX=3 so 2 TP → ~67%
precision (clears king ~47% by >5%).
"""
...
DEEP_N = 7
WIDE_N = 10
# 2 TP / 3 emits ≈ 67% precision → beat king ~47% by >5% (tier 5 win).
# Prefer precision over F1: F1 depends on planted-bug denominator which swings.
# #186 lost PASS by changing the call path, not by this cap alone.
EMIT_MAX = 3
DESC_MIN = 40
# Keep #184 clock that delivered the PASS (do not retune).
WALL = 660.0
HTTP_TO = 175.0
RESERVE = 255.0
POST = 12.0
