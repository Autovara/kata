"""SN60 Bitsec challenger — #187 PASS/0-invalid + TP+1 kill shot.

#187 scoreboard (decided at TP — one bug short of the crown):
  pass 14% vs 6% TIE | projects 1 vs 0.4 TIE | TP 2 vs 2.1 BEHIND ← lost
  invalids 0 vs 2.1 would WIN (lead 2.1 > 2) if TP were not behind
  precision 25% / F1 10% never reached

Win completely: keep #187 audit path + 0-invalid clock, raise recall so
TP >= 3 (ties king 2.1), then take the crown on invalids. Also lift emit
cap and soften over-filters that can drop a real third TP.
"""
...
DEEP_N = 8
WIDE_N = 12
# #187's emit-3 likely truncated a 3rd TP. Room for ≥3 TP; invalids decide.
EMIT_MAX = 6
DESC_MIN = 40
# Keep #187 clock that delivered PASS + 0 invalids.
WALL = 660.0
HTTP_TO = 175.0
RESERVE = 255.0
POST = 12.0
...
DEEP_TOK = 16_000
WIDE_TOK = 14_000
# Slightly earlier wide pass → more recall, still under the 0-invalid budget.
THIRD_SLACK = 110.0
...
def normalize(raw: dict[str, Any], by_rel: dict[str, dict[str, Any]],
    ...
    try:
        ...
        conf = 0.7
    # Soft floor — #180's hard gate wiped TPs; #187 needed one more TP.
    if conf < 0.35:
        return None
    # Prefer localized, but keep title-recovered / last-resort findings.
    if not function and not raw.get("_allow_unlocalized"):
        # Still emit if description is rich enough for the judge to match.
        if len(description) + len(mechanism) + len(impact) < 120:
            return None
    conf = max(conf, 0.55)
...
def _dedupe(items: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    out: list[dict[str, Any]] = []
    # Recall-first rank: keep localized/critical/high-conf, but do NOT drop
    # unlocalized until the emit budget is full — a dropped TP loses the crown.
    for f in sorted(
            items,
            key=lambda x: (
                bool(x.get("function")),
...
def agent_main(project_dir: str | None = None, inference_api: str | None = None) -> dict:
    ...
    try:
        ...
        if deadline - time.monotonic() >= RESERVE:
            try:
                # Slightly wider deep batch than #187 — chase the missing 3rd TP.
                n = min(len(ordered), len(ordered) if tiny else (10 if compact else DEEP_N))
        ...
        # Always merge high-signal probes — need TP >= 3 to clear tier 3.
        # Invalids (0 vs ~2.1) then win the crown; precision is secondary.
        try:
            raw.extend(probes(recs)[:3])
        except Exception:
            pass
