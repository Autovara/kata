"""SN60 / Bitsec miner agent — hybrid depth-then-breadth.

Lesson from the prior round: on a large sample, feeding the model one whole
contract at a time (depth) finds more real vulnerabilities than spreading a
fixed character budget across many truncated contracts (breadth). This agent
keeps the strengths of both within the validator's hard per-problem inference
budget (3 successful model calls / 24k output tokens):

  * calls 1-2 are FULL-DEPTH audits of the two highest-risk contracts — the
    whole file (untruncated up to a generous cap) plus one directly-imported
    dependency for context, so the model can follow the protocol logic;
  * call 3 is a BREADTH pass over the next tier of suspicious contracts, so a
    bug outside the top two is still reachable.
...
timeout, a budget ``429``, or a malformed reply simply yields the findings
gathered so far.
"""
...
def _related_source(
    ...
    """Pull one directly-imported local file for extra context on a depth call."""
...
def agent_main(
    ...
    try:
        ...
        # Build the call plan: up to DEPTH_TARGETS single-contract depth calls,
        # then one breadth call over the next tier — never exceeding MAX_CALLS.
