...
def _resolve_file(want, by_base, recs_by_rel):
    ...
    for rel, rec in recs_by_rel.items():
        if rel.endswith(want) or want.endswith(rel):
            return rec
    return None
...
def _pick_function(candidates, funcs, title, desc):
    ...
    #      the description carries the true location, so trust the source.
    for hay in (title or "", desc or ""):
        for fn in sorted(funcs, key=len, reverse=True):
            if len(fn) >= 4 and fn in hay:
                return fn
...
def _reference_findings(recs):
    ...
    for proj in _TABLE:
        ...
        for f in proj["f"]:
            ...
            contract = f["contract"] or (rec["contracts"][0] if rec["contracts"] else rec["stem"])
