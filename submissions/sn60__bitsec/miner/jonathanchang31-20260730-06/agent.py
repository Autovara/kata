...
#: Sized from what the passes actually YIELD, not from what the room allows. Every finding this
#: agent has produced came from the structural probes, which run first and finish early; the broad
#: survey/focus passes that consumed the rest of the budget contributed none. Burning the room's
#: full clock re-reading files that yield nothing costs the miner money and makes every screening
#: gate and every scored replica slower for no gain.
WALL_BUDGET = _num("AGENT_WALL_BUDGET", 420.0)
TAIL_RESERVE = _num("AGENT_TAIL_RESERVE", 30.0)
PER_CALL_TIMEOUT = _num("AGENT_CALL_TIMEOUT", 150.0)
...
FOCUS_FILES = _int("AGENT_FOCUS_FILES", 3)
PROBE_LIMIT = _int("AGENT_PROBE_LIMIT", 10)
DESYNC_LIMIT = _int("AGENT_DESYNC_LIMIT", 6)
MAX_FINDINGS = _int("AGENT_MAX_FINDINGS", 40)

#: A confirmed finding measured ~400 tokens. The old 3000 ceiling bought no better analysis:
#: generation latency scales with tokens PRODUCED, so an oversized cap only let a verbose model
#: ramble for minutes and, at worst, get truncated before writing its JSON at all.
PROBE_TOKENS = _int("AGENT_PROBE_TOKENS", 1800)
FOCUS_TOKENS = _int("AGENT_FOCUS_TOKENS", 2000)
SURVEY_TOKENS = _int("AGENT_SURVEY_TOKENS", 2200)
...
def run_jobs(jobs: list[tuple[str, str, int]], endpoint: str, deadline: float) -> list[str]:
    """Run prompts concurrently on DAEMON threads, returning whatever finished in time.

    The daemon flag is the load-bearing part, and it is not a detail. The runner executes this
    agent inside a ``multiprocessing`` child and reads the result off a queue; the child has to be
    able to EXIT for that result to be delivered. ``ThreadPoolExecutor`` uses non-daemon workers and
    registers an ``atexit`` hook that JOINS them, so a single worker parked in a socket read holds
    the interpreter open past the point where anything is still watching. The run then looks, from
    outside, exactly like an agent that finished and produced nothing -- exit code 0, no report --
    even though the analysis completed and the inference was paid for.

    Daemon threads cannot do that: the interpreter exits regardless of what they are still waiting
    on. The bounded join below means this function also returns on the deadline rather than on the
    slowest straggler, so a wedged upstream costs the calls it owns and nothing more.
    """
    results: list[str] = []
    if not jobs:
        return results
        ...
        while True:
            with lock:
                ...
                if index >= len(jobs):
                    return
                cursor[0] += 1
            if time.monotonic() > deadline - TAIL_RESERVE:
                return
            _label, prompt, tokens = jobs[index]
            try:
                text = ask(endpoint, prompt, deadline, tokens)
    ...
    threads = [
        threading.Thread(target=worker, daemon=True, name="kata-agent-%d" % i)
...
def agent_main(project_dir: str | None = None, inference_api: str | None = None) -> dict:
    ...
    report = {"vulnerabilities": select(merge(findings))}
    # Written HERE rather than from a ``__main__`` block, because the runner IMPORTS this module
    # and calls ``agent_main()`` -- under an import ``__name__`` is "agent", so anything guarded by
    # ``if __name__ == "__main__"`` never executes. The room then finds no report to copy out of
    # the container and reports "completed without writing report.json", which looks like a crash
    # and is not one: the agent ran, called inference successfully, and simply had nowhere to put
    # its answer. Doing it from inside the function covers the import path AND the script path.
    write_report(report)
    return report
...
def write_report(report: dict) -> str | None:
    ...
    """
    payload = json.dumps({"success": True, "report": report})
    # EVERY location the report is known to be collected from, not just the configured one.
    # ``REPORT_FILE`` is the documented contract, but an agent that trusts it alone writes nothing
    # when the variable is absent from its container -- and then the run looks like a crash despite
    # having completed and paid for real inference. The runner copies the report out of a fixed
    # path, so writing there too costs one small file and removes an entire class of silent loss.
    written: list[str] = []
    targets = []
    configured = os.environ.get("REPORT_FILE", "").strip()
    if configured:
        targets.append(configured)
    targets.extend(["/kata_output/report.json", "/app/report.json", "report.json"])
    seen = set()
    for target in targets:
        ...
        try:
            ...
            continue        # a path this container cannot write is simply not this container's
