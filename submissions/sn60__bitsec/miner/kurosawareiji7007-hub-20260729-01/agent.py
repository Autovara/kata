"""SN60 / Bitsec miner — parallel depth-first vulnerability hunter.

Whole-repository high/critical audit across Solidity, Vyper, Rust (Solana/Anchor +
CosmWasm), Move, and Cairo.

Design
------
Project-pass is decided first, so recall dominates: missing one planted issue on a
project cannot be recovered elsewhere. This agent keeps the king's corroboration +
localization guard, but schedules *per-file* deep reviews concurrently so more of
the ranked tree is actually read inside one wall-clock budget. Each LLM call still
targets a focused slice (one primary file plus import neighbours), failures drop
silently, and cross-pass agreement promotes the strongest-supported findings.
"""
...
def run_parallel_deep(inference_api, ordered, by_base, deadline, per_file_cap, budget):
    """Run one deep review per top-ranked file concurrently."""
    if not ordered or deadline - time.monotonic() < CALL_HEADROOM:
        return []
    lock = threading.Lock()
    merged: list[dict] = []
    ...
    def one_file(rec):
        if deadline - time.monotonic() < CALL_HEADROOM:
            return []
        try:
            return run_deep(inference_api, [rec], by_base, deadline, per_file_cap, budget)
        except Exception:
            return []

    workers = min(PARALLEL_WORKERS, len(ordered[:DEEP_FILE_COUNT]))
    with ThreadPoolExecutor(max_workers=workers) as pool:
        futures = [pool.submit(one_file, rec) for rec in ordered[:DEEP_FILE_COUNT]]
...
def agent_main(project_dir=None, inference_api=None):
    ...
    try:
        ...
        if deadline - time.monotonic() >= CALL_HEADROOM:
            try:
                raw.extend(run_parallel_deep(
                    inference_api, ordered[:DEEP_FILE_COUNT], by_base,
