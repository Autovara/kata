...
def agent_main(project_dir=None, inference_api=None):
    ...
    try:
        ...
        # ALWAYS call inference when a gateway is injected — screening and scoring
        # observe whether the agent hits /inference. Static hits stay as backup.
        if endpoint and deadline - time.monotonic() > FLOOR + 50:
            try:
                ...
                pass
            # Second short pass on the next file slice when budget remains.
            if deadline - time.monotonic() > FLOOR + 50:
                try:
                    batch2 = rows[FOCUS_N:FOCUS_N + FOCUS_N] or rows[:FOCUS_N]
                    text2 = _call(endpoint, _focus_prompt(batch2), deadline, TOK_FOCUS)
                    raw.extend(_findings(text2))
                except Exception:
                    pass
