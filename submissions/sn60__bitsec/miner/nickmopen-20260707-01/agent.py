...
MAX_FILES = 60
MAX_CALL_CHARS = 16_000       # source context packed per audit call
MAX_FILE_FULL = 4_500         # files <= this are sent whole; larger are compacted
MAX_FILE_COMPACT = 3_200      # cap for a compacted (signatures + risky bodies) file
MAX_FINDINGS = 10
...
def _batches(records: list[dict[str, Any]]) -> list[list[dict[str, Any]]]:
    """Pack the highest-risk contracts (compacted) into up to MAX_MODEL_CALLS calls,
    maximizing how many distinct files the model actually audits within budget."""
    ...
    for rec in records:
        ...
        clen = len(rec["ctx"])
        if cur and used + clen > MAX_CALL_CHARS:
            batches.append(cur)
            cur, used = [], 0
        ...
        cur.append(rec)
        used += clen
    if cur and len(batches) < MAX_MODEL_CALLS:
        batches.append(cur)
    return batches[:MAX_MODEL_CALLS]
