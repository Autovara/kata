...
def agent_main(project_dir: str | None = None, inference_api: str | None = None) -> dict:
    ...
    planning = _json_object(_request(endpoint, _planning_prompt(records), max_tokens=5_000))
    primary, secondary = _audit_batches(planning, records, graph)
    raw: list[dict[str, Any]] = []

    if _has_time(started):
        raw.extend(
            _audit(
                endpoint,
                primary,
...
def _skip_path(path: Path, root: Path) -> bool:
    ...
    for part in relative.parts[:-1]:
        lowered = part.lower()
        if lowered in SKIP_DIRECTORIES or lowered.startswith("."):
            return True
    return relative.name.lower().endswith((".t.sol", ".s.sol", "_test.sol", ".test.sol"))
...
def _audit_prompt(
    ...
    return (
        ...
        + _source_pack(records, graph, source_index)
    )
...
def _source_pack(
    ...
    for index, record in enumerate(records):
        ...
        block = _record_block(record, allowance)
        ...
        for neighbor in _neighbors(record, records, graph, source_index)[:2]:
            support = _related_block(neighbor, min(RELATED_CHARS, remaining))
            if support:
...
def _record_block(record: dict[str, Any], allowance: int) -> str:
    metadata = (
        f"\n\n===== FILE {record['rel']} =====\n"
    ...
    return metadata + _source_excerpt(record, max(1_000, allowance - len(metadata)))


def _related_block(record: dict[str, Any], allowance: int) -> str:
    if allowance < 1_000:
        return ""
    header = f"\n\n----- RELATED FILE {record['rel']} -----\n"
    return header + _source_excerpt(record, max(800, allowance - len(header)))


def _source_excerpt(record: dict[str, Any], limit: int) -> str:
    ...
    functions = sorted(
        record["functions"],
        key=lambda item: (-int(item["priority"]), int(item["line"]), str(item["name"])),
    )
...
def _request(endpoint: str, prompt: str, *, max_tokens: int) -> str:
    if not endpoint:
        return ""
    body = json.dumps(
        {
    ...
    except (OSError, ValueError, TimeoutError, urllib.error.HTTPError, urllib.error.URLError):
        return ""
