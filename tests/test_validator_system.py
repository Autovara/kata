...
def test_resolve_sn60_project_keys_is_deterministic(tmp_path: Path, monkeypatch) -> None:
    sandbox_root = tmp_path / "sandbox"
    benchmark = write_benchmark(sandbox_root)
    monkeypatch.setenv("KATA_SN60_PROJECT_SAMPLE_SIZE", "2")
    monkeypatch.setenv("KATA_SN60_PROJECT_SAMPLE_SECRET", "secret")
    ...
    first = resolve_sn60_project_keys(**kwargs)
    second = resolve_sn60_project_keys(**kwargs)
